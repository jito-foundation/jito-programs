...
use jito_tip_distribution::{
    ...
    state::{Config, MerkleRoot, TipDistributionAccount},
};
...
use crate::helpers::merkle_tree::MerkleTree;
use crate::CLAIM_STATUS_SEED;
use crate::ROOT_UPLOAD_CONFIG_SEED;
...
pub fn assert_distribution_account(
    actual: &TipDistributionAccount,
    expected: &TipDistributionAccount,
) {
    assert_eq!(
...
pub fn derive_tip_distribution_account_address(vote_pubkey: &Pubkey, epoch: u64) -> (Pubkey, u8) {
...
pub fn derive_tip_distribution_account_address_with_id(
    program_id: &Pubkey,
    vote_pubkey: &Pubkey,
    epoch: u64,
) -> (Pubkey, u8) {
...
pub fn derive_config_account_address() -> (Pubkey, u8) {
    Pubkey::find_program_address(&[Config::SEED], &jito_tip_distribution::id())
}
...
pub async fn setup_init_tip_distribution_account(
    ...
    let validator_identity = generate_account(ctx, 10_000_000_000_000).await;
    ...
    let (tip_distribution_account, bump) = derive_tip_distribution_account_address_with_id(
        program_id,
        &validator_vote_account.pubkey(),
        epoch,
    );
...
pub async fn call_init_tip_distribution_account(
    ...
    let ix = instruction::InitializeTipDistributionAccount {
        merkle_root_upload_authority: *merkle_root_upload_authority,
        validator_commission_bps,
        bump,
    };

    let accounts = accounts::InitializeTipDistributionAccount {
        config: *config,
        system_program: system_program::id(),
...
pub async fn call_close_tip_distribution_account(
    ctx: &ProgramTestContext,
    config: &Pubkey,
    expired_funds_account: &Pubkey,
    tip_distribution_account: &Pubkey,
    ...
    let ix_data = instruction::CloseTipDistributionAccount { _epoch: epoch };

    let accounts = accounts::CloseTipDistributionAccount {
        config: *config,
        expired_funds_account: *expired_funds_account,
        tip_distribution_account: *tip_distribution_account,
        validator_vote_account: *validator_vote_account,
...
pub async fn call_claim(
    ...
    let tx = Transaction::new_signed_with_payer(
        &[ix],
        Some(&ctx.payer.pubkey()),
        &[&ctx.payer, payer, merkle_root_upload_authority],
        ctx.last_blockhash,
    );
...
pub async fn setup_with_uploaded_merkle_root(
    ...
    let accounts = accounts::UploadMerkleRoot {
        tip_distribution_account: setup.tip_distribution_account,
