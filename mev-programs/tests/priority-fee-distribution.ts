...
describe("tests priority_fee_distribution", () => {
  ...
  it("#init_tip_distribution_account happy path", async () => {
    ...
    // expect
    const actual = await priorityFeeDistribution.account.tipDistributionAccount.fetch(
      tipDistributionAccount,
    );
    const expected = {
  ...
  it("#close_tip_distribution_account happy path", async () => {
    ...
    const tda = await priorityFeeDistribution.account.tipDistributionAccount.fetch(
      tipDistributionAccount,
    );
  ...
  it("#claim happy path", async () => {
    ...
    const user0Info = await priorityFeeDistribution.provider.connection.getAccountInfo(
      user0.publicKey,
    );
...
const setup_initTipDistributionAccount = async () => {
  // Fetch config state.
  const config = await priorityFeeDistribution.account.config.fetch(configAccount);
