...
describe( 'tests tip_distribution', () => {
    ...
    it('#close_tip_distribution_account happy path', async () => {
        ...
        const actualConfig = await tipDistribution.account.config.fetch(configAccount)
        const tda = await tipDistribution.account.tipDistributionAccount.fetch(tipDistributionAccount);

        const balStart = await provider.connection.getBalance(validatorVoteAccount.publicKey);
        ...
        const balEnd = await provider.connection.getBalance(validatorVoteAccount.publicKey);

        const minRentExempt = await provider.connection.getMinimumBalanceForRentExemption(TIP_DISTRIBUTION_ACCOUNT_LEN);
        assert(balEnd - balStart === minRentExempt)

        try {
            // cannot fetch a closed account
            await tipDistribution.account.tipDistributionAccount.fetch(tipDistributionAccount);
    ...
    it('#upload_merkle_root happy path', async () => {
        ...
        try {
            await tipDistribution.rpc.uploadMerkleRoot(
                root.toJSON().data, maxTotalClaim, maxNumNodes,
                {
                    accounts: {
                        tipDistributionAccount,
                        merkleRootUploadAuthority: validatorVoteAccount.publicKey,
    ...
    it('#claim happy path', async () => {
        ...
        const user0Info = await tipDistribution.provider.connection.getAccountInfo(user0.publicKey);
    ...
    it('#close_claim_status works even if TipDistributionAccount already closed', async () => {
        ...
        const minRentExempt = await provider.connection.getMinimumBalanceForRentExemption(CLAIM_STATUS_LEN);
        assert(balEnd - balStart === minRentExempt)
    })
...
const generateAccount = async (airdropAmount: number) => {
    ...
    if (airdropAmount) {
        await provider.connection.confirmTransaction(
            await provider.connection.requestAirdrop(
                account.publicKey, airdropAmount
            ),
            'confirmed',
        )
    }
