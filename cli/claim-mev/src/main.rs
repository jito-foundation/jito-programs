...
fn command_claim_all(
    ...
    for tree in &merkle_tree.generated_merkle_trees {
        ...
        for node in &tree.tree_nodes {
            ...
            total_claims += 1;
            if let Some(claim) = query_claim_status(rpc_config, &claim_status) {
                if claim.is_claimed {
            ...
            match send_transaction(rpc_config, &[ix.clone()], payer) {
                Ok((_, signature)) => match query_claim_status(rpc_config, &claim_status) {
                    Some(claim_status) => {
    ...
    println!(
        "Total Claims: {}, Previously Claimed: {}, Claimed: {}, Claim Errors: {}",
        total_claims,
        previously_claimed,
        successful_claims,
...
fn query_claim_status(rpc_config: &RpcConfig, claim_status: &Pubkey) -> Option<ClaimStatus> {
    if let Ok(account_resp) = rpc_config
        .rpc_client
